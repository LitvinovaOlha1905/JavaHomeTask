package de.telran.lesson9.scoreboardwithclock;

import java.util.Random;

public class ScoreboardWithClock {
    ...
    /*Есть устройство, на табло которого показывается количество секунд, оставшихся до конца рабочего дня.
          ...
          «остался 1 час», «осталось менее часа». Обьяснение: в переменную n должно записываться случайное
          (на время тестирования программы) целое число из диапазона от 0 до 28800, далее оно должно выводиться на экран
          (для тех, кто понимает в секундах) и на следующей строке (для тех кто не понимает) должна выводиться фраза
    ...
    public static void main(String[] args) {
        ...
        // Генерируем случайное число от 0 до 28800 (8 часов)
        int hoursInSeconds = value.nextInt(28801);

        // Выводим количество секунд на табло
        System.out.println(hoursInSeconds);

        // Вычисляем количество полных часов
        int fullHours = hoursInSeconds / 3600;
        System.out.println(fullHours);

        // Выводим информацию о полных часах на табло
        if (fullHours > 0) {
            System.out.println("До конца рабочего дня осталось секунд: " + hoursInSeconds);
            System.out.println("До конца рабочего дня осталось часов: " + fullHours);
        }else {
            System.out.println("До конца рабочего дня осталось секунд: " + hoursInSeconds);
